...
for SEED in [0, 4, 5, 6, 7, 8, 9]:
    ...
    WARM_UP_STEPS = 500 #1000 #0# number of timesteps before training starts
    TAU = 1e-4
    LR = 1e-4
    ...
    folder = r'/Users/nikolaibeckjensen/Desktop/metro_results'
    sub_dir = f"Seed{SEED}_Actions{MAX_ACTIONS}"
    path = os.path.join(folder, sub_dir)
    os.makedirs(path, exist_ok=True)

    # save all parameters to a json file in path
    params = {
        "SEED": SEED,
        ...
        "TAU": TAU,
        "LR": LR,
        "INSPECT_FINAL_SOLUTION": INSPECT_FINAL_SOLUTION,
        "N_ACTIONS": N_ACTIONS,
    ...
    def select_action(state, mask):
        ...
        if len(available_actions) == (N_ACTIONS - MAX_ACTIONS + 1):
            print("------------ FINAL ACTION ------------")
            final = True
    ...
    for i_episode in range(NUM_EPISODES):
        ...
        for t in count():
            ...
            if done:
                episode_scores.append(min(mediator.score, 300))
            ...
            # show solution for final 10 episodes
        if i_episode > NUM_EPISODES - 10:
            ...
            # save screen image
            img_save_path = os.path.join(path, f"final_solution_{i_episode}.png")
            pygame.image.save(screen, img_save_path)

    print('Complete')

    # save final 100 episode scores
    json_path = os.path.join(path, "episode_scores.json")
    with open(json_path, 'w') as f:
    ...
    save_path = os.path.join(path, f"Seed{SEED}_Actions{MAX_ACTIONS}.png")
